...
__device__ u32 pt2MaxG = 0;
__device__ u32 pt3MaxG = 0;
...
__global__ void fileEncryption128counterWithOneTableExtendedSharedMemoryBytePermPartlyExtendedSBox(u32* pt, u32* ct, u32* rk, u32* t0G, u32* t4G,  
	...
	for (;;) {
		...
		atomicMax(&pt2MaxG, pt2Max);
		atomicMax(&pt3MaxG, pt3Max);
	...
	if (fileIn) {
		...
		while (1) {
			...
			if (fileIn) {
				...
				//printf("Only %ld characters could be read.\n", readByte);
			}
			// Process read buffer
			u32 readInt = 0;
			for (int bufferIndex = 0; bufferIndex < readByte; bufferIndex++) {
				...
				if (bufferIntIndex == 0) {
					...
					//printf("ct index: %d   ct: %08x\n", bufferIndex / U32_SIZE, ct[bufferIndex / U32_SIZE]);
